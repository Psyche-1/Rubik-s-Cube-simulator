...
import re, os, shelve, pprint#, graphics as gr
...
def find_different(cube_1, cube_2): #нет реализации в main
	'''
...
#'''
		elif file_type == 'cube_save_pyfile':

			if not os.path.isfile(os.path.join(saving_directory, 'Cube_saves_file.py')):
				cube_save_pyfile = open('Cube_saves_file.py', 'a')
				if where_saving in ['Save_to_last', 'Save_to_mine', 'Temporary_cube']: #, 'Save_start'
					if turn_string != 'Empty':
						cube_save_pyfile.write( 'turn_string' + ' = ' + pprint.pformat(turn_string) + ')\n\n' )
					cube_save_pyfile.write( where_saving + ' = (\n' + pprint.pformat(cube_list_for_saving) + ')\n\n\n\n' )

				cube_save_pyfile.close()
...
#"""
##saving_cube(cube_list, 'Save_start', 'n', file_type = 'cube_save_pyfile')
#saving_cube(cube_list, 'Save_to_last', 'n', file_type = 'cube_save_pyfile')
#saving_cube(cube_list, 'Save_to_mine', 'n', file_type = 'cube_save_pyfile')
...
def loading_cube(wherefrom_loading = 'Loading_from_last', file_type = 'shelf_file'):
	...
	if  wherefrom_loading == 'Loading_from_start':
		cube_list_from_save = start_cube_list
