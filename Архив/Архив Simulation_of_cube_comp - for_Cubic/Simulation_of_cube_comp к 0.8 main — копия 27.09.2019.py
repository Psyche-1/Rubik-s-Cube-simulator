...
def saving_cube(cube_list_for_saving,
	...
	if with_txt in ['completely', 'c', 'y', 'yes', 'only']:
		...
		Cube_saves_file.write(	wherefrom_string + '\n\n' +
								turn_string + ')' + '\n\n' +
								super_cube + '\n\n\n\n'
								)
	...
	if with_txt != 'only':
		if file_type == 'shelf_file':
			...
			if where_saving in ['Save_to_last', 'Save_to_mine', 'Save_start', 'Temporary_cube']:
		...
		elif file_type == 'cube_save_file':

			if not os.path.isfile(os.path.join(saving_directory, 'Cube_saves_file.py')):
				cube_save_file = open('Cube_saves_file.py', 'a')
				if where_saving in ['Save_to_last', 'Save_to_mine', 'Save_start', 'Temporary_cube']:
					cube_save_file.write( where_saving + ' = (\n' + pprint.pformat(cube_list_for_saving) + ')\n\n\n' )
				cube_save_file.close()

			else:
				pass
			#import Cube_saves_file # Косяк с перезаписью переменных, тк дописывает их #????

			#Один раз сохранить, а потом закомментить либо оставить и запускать, чтобы точно был начальным #?


#"""
#saving_cube(cube_list, 'Save_start', 'n', file_type = 'cube_save_file')
#saving_cube(cube_list, 'Save_to_last', 'n', file_type = 'cube_save_file')
#saving_cube(cube_list, 'Save_to_mine', 'n', file_type = 'cube_save_file')



def loading_cube(wherefrom_loading = 'Loading_from_last', file_type = 'shelf_file'):
	'''
	...
	if file_type == 'shelf_file':
		...
		if wherefrom_loading == 'Loading_from_last':
			cube_list_from_save = shelf_file['Save_to_last'] # Загрузка

		elif  wherefrom_loading == 'Loading_from_mine':
			cube_list_from_save = shelf_file['Save_to_mine'] # Загрузка

		elif  wherefrom_loading == 'Loading_from_start':
			pass
			cube_list_from_save = shelf_file['Save_start'] # Загрузка
		shelf_file.close


	elif file_type == 'cube_save_file':
		import Cube_saves_file

		if wherefrom_loading == 'Loading_from_last':
			cube_list_from_save = Cube_saves_file.Save_to_last

		elif wherefrom_loading == 'Loading_from_mine':
			cube_list_from_save = Cube_saves_file.Save_to_mine

		elif wherefrom_loading == 'Loading_from_start':
			cube_list_from_save = Cube_saves_file.Save_start
