...
Сделать через дизайнер, что-то типа того что в калькуляторе
либо сразу вращать, либо хотя бы для простой записи вращений по нажатию на кнопку 
"""
...
def check_string(main_string):
	...
	Как: через регулярные выражения если есть символы не из списка [a-zA-Z()' 2^]
	Возможно выбрать только конкретные буквы
	'''
...
string = "U2 D2 E2 F2 B2 S2 R2 L2 M2"#input()'Standing'
