...
Дата последней редакции кода: 27.09.2019. Версия 0.8 main				ПРОВЕРИТЬ НА БАГИ И ДОДЕЛАТЬ #?
...
Добавить: #?
	Проверять все принты при запуске: Отправляють print в Sms_as_cmd (sms_as_cmd_display = print + 2)
...
def rotation(turn):
	...
	if Entering_cube or Entering_cube_for_loading or Typing_rotate:
		...
		if turn in ['U', 'F', 'R', 'D', 'B', 'L' ,'E' ,'S' ,'M']:
			...
			print('Error in rotation. In Rotating_immediately mode')
	...
	if Entering_cube or Entering_cube_for_loading or Typing_rotate:
		...
		if text == '':
			...
			if sign == "'":
				...
				print('Error in bp_Backspace. In Rotating_immediately mode')
...
def bp_Enter(): # Проверить как лучше с сохранением и запуском в обоих режимах #?
	...
	'''
	"""
	# Rotation painting cube # Оставить на всякий поддержку рисовалки
	global cube_list
