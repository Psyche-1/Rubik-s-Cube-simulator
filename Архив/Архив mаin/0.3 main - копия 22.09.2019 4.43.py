...
1.3 Вывод кубика из сохранения(def cube_list_to_buttons_cube)
...
def sms_as_cmd_print(): #Добавить добавление текста вместо принт #?
...
def save(cube_list_for_saving=[], with_txt='No'): #????
	SCube.saving_cube(cube_list_for_saving, with_txt)
...
def radio_button():
	...
	if ui.Enter_cube.isChecked():
		...
		text_for_sms_as_cmd = 'Enter cube Using\n\n'
		sms_as_cmd_print()
	...
	if ui.Type_rotate.isChecked():
		...
		text_for_sms_as_cmd = 'Typing rotate Using\n\n'
		sms_as_cmd_print()
	...
	if ui.Rotate_immediately.isChecked():
		...
		text_for_sms_as_cmd = 'Rotate immediately Using\n\n'
		sms_as_cmd_print()
...
#'''
def bp_00(test='0'):  #Попробовать сделать в одну функцию, а не 54! #???
	# Через bp_(Side_Square, test='0'):
	#Если получится сделать одной функцией, то добавить #?
	#if b_coloring == '':
	#	print('Choose color') вместо того чтобы присваивать и вписывать пустую строку
	#if test == '0':
	#	testing = 'failed'
	#else:
	#	print(str(test))
