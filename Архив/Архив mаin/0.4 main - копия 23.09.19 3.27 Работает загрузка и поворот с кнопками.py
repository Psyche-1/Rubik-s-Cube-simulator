...
def sms_as_cmd_print():
	...
	#else:
	#	text_from_Sms_as_cmd = ui.Sms_as_cmd.text() #Проверить что нет так пишет не поддерживает .text #?????
...
def Choose_mode():
	...
	if ui.Rotate_immediately.isChecked():
		print("Using Rotate immediately\n\n")
...
#	global text_for_sms_as_cmd
	...
	if ui.Load_from_start.isChecked():
		print('Using Loading from start\n')
...
def cube_list_to_buttons_cube(): # Перекинуть ближе к концу, для избежания ошибок #?
	#'''
	#pass
	global cube_list
	...
	colors = {'y': 'yellow', 'o': 'orange', 'b': 'blue', 'r': 'red', 'w': 'white','g': 'green'}
	for side in cube list
		for square in side
			square = #(по словарю замена у на yellow и тд)
			bp_coloring(square)
			bp_SS(str(side) + str(square)) # Придумать как выбирать номер кнопки # Похоже так!!!!!!!!!!
	
	
	
	Копия пример из SCube
	Или SCube.coloring(cube_list, 1), а после SCube.coloring(cube_list, 2)
	colors = {'y': 'yellow', 'o': 'orange', 'b': 'blue', 'r': 'red', 'w': 'white','g': 'green'}
...
def save(cube_list_for_saving=[], with_txt='No'): #????
	...
	if cube_list_for_saving == []:
		print('cube_list_for_saving is empty. Saving cube_list')
		text_for_sms_as_cmd = 'cube_list_for_saving is empty. Saving cube_list\n\n'
		sms_as_cmd_print()
	...
	if Saving_to_last:
		...
		print('Chose what saving')
		text_for_sms_as_cmd = 'Chose what saving\n\n'
		sms_as_cmd_print()
